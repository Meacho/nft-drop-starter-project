...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    //toGMTsting is depreceted so changed to toUTCString
    const goLiveDateTimeString = `${new Date(goLiveData * 1000).toUTCString()}`;
    // add local time countdown as well
    const goLiveDateTimeToLocalString = `${new Date(
      goLiveData * 1000
    ).toLocaleDateString()} at ${new Date(
      goLiveData * 1000
    ).toLocaleTimeString()}`;

    //add items to the useState to render
    setCandyMachine({
  ...
  const renderDropTimer = () => {
    //Get current date and dropdate in JS Data object
    const currentDate = new Date();
    const dropDate = new Date(candyMachine.state.goLiveData * 1000);
    console.log(dropDate);
    if (currentDate < dropDate) {
      console.log("Before dropdate");
    ...
    candyMachine.state && (
      <div className="machine-container">
        ...
        <div className="time">
          {" "}
          {renderDropTimer()}
          <p>{`Drop Date (UTC): ${candyMachine.state.goLiveDateTimeString}`}</p>
          <p>{`Drop Date (Local): ${candyMachine.state.goLiveDateTimeToLocalString}`}</p>
